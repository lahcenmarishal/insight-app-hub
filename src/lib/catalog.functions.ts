// Browser-side replacement for the former TanStack server functions.
// All operations go through the publishable Supabase client and rely on RLS
// (admin policies via has_role for mutations, public SELECT for reads).
...
export async function fetchCatalog() {
  ...
  const products = (prods.data ?? []).map((row) => {
    const p = mapProduct(row as Record<string, unknown>);
    return {
      ...p,
      image: resolveAssetUrl(p.image),
      datasheetUrl: p.datasheetUrl ? resolveAssetUrl(p.datasheetUrl) : undefined,
      gallery: (p.gallery ?? []).map(resolveAssetUrl).filter(Boolean),
    };
  });
  return {
    categories: (cats.data ?? []).map((r) => mapCategory(r as Record<string, unknown>)),
    products,
    suppliers: (sups.data ?? []).map((r) => mapSupplier(r as Record<string, unknown>)),
  };
}

// ============= Products (admin) =============

type ServerArg<T> = { data: T };

export async function saveProductFn({ data }: ServerArg<AdminProduct>) {
...
export async function deleteProductFn({ data }: ServerArg<{ id: string }>) {
  const { error } = await supabase.from("products").delete().eq("id", data.id);
  if (error) throw new Error(error.message);
  return { ok: true };
}

export async function setArchivedFn({ data }: ServerArg<{ id: string; archived: boolean }>) {
...
export async function uploadProductAssetFn({
  data,
}: ServerArg<{
  productId: string;
  kind: "image" | "gallery" | "datasheet";
  filename: string;
  contentType: string;
  dataBase64: string;
}>) {
  ...
  return { token: `${STORAGE_PREFIX}${path}`, signedUrl: pub?.publicUrl ?? "" };
}

// ============= Suppliers (admin) =============

export async function saveSupplierFn({
  data,
}: ServerArg<Omit<Supplier, "id"> & { id?: string }>) {
  const row = {
    name: data.name,
  ...
  if (data.id) {
    const { error } = await supabase.from("suppliers").update(row).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { id: data.id };
  }
...
export async function deleteSupplierFn({ data }: ServerArg<{ id: string }>) {
...
export async function submitQuoteFn({ data }: ServerArg<QuoteSubmission>) {
  ...
  if (productIds.length) {
    ...
    (prods ?? []).forEach((p: { id: string; sale_price: number | null }) =>
      priceMap.set(p.id, Number(p.sale_price ?? 0)),
    );
  }
  ...
  return { id: quote.id as string, totalHt };
}
...
export async function fetchQuotesFn(): Promise<QuoteRow[]> {
  ...
  return (quotes ?? []).map((q: Record<string, unknown>): QuoteRow => ({
    id: q.id as string,
...
export async function signQuoteFn({
  data,
}: ServerArg<{ id: string; signerName: string; signatureData: string }>) {
  const { error } = await supabase
    .from("quotes")
    .update({
...
export async function updateQuoteStatusFn({
  data,
}: ServerArg<{ id: string; status: "Nouveau" | "En cours" | "Envoyé" | "Signé" | "Refusé" }>) {
  const { error } = await supabase
    .from("quotes")
...
export async function updateQuoteItemPricesFn({
  data,
}: ServerArg<{
  quoteId: string;
...
const LOGO_PATH = "/assets/innova-logo.png";

export async function generateQuotePdfFn({ data }: ServerArg<{ id: string }>) {
...
export async function fetchProspectsFn(): Promise<Prospect[]> {
  const { data, error } = await supabase
    .from("prospects")
    .select("*")
    .order("created_at", { ascending: false });
  if (error) throw new Error(error.message);
  return (data ?? []).map((r) => mapProspect(r as Record<string, unknown>));
}
...
export async function saveProspectFn({ data }: ServerArg<ProspectInput>) {
  const row = {
    company: data.company,
    contact: data.contact,
    phone: data.phone,
  ...
  if (data.id) {
    const { error } = await supabase.from("prospects").update(row).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { id: data.id };
  }
  const { data: ins, error } = await supabase
...
export async function deleteProspectFn({ data }: ServerArg<{ id: string }>) {
  const { error } = await supabase.from("prospects").delete().eq("id", data.id);
  if (error) throw new Error(error.message);
  return { ok: true };
}
